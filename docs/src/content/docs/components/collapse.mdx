...
<PreviewTopCodeBottom>
  ...
  <CodeLoader source="CollapseExample1" noRoundedTop/>
</PreviewTopCodeBottom>
...
<PreviewTopCodeBottom>
  ...
  <CodeLoader source="MultipleCollapse" noRoundedTop/>
</PreviewTopCodeBottom>
...
Update the collapsed height programmatically.

```js
myCollapse.setCloseHeight(80);
```

### `cleanup`

Remove the event listeners attached by the component when you no longer need the instance.
...
The collapse element dispatches custom events that you can listen to from your application code.

### `before-expand`

Fires right before the element is expanded.

### `expanded`

Fires after the element has been expanded.

### `collapsed`

Fires after the element has been collapsed.

```js
const collapseEl = document.querySelector("#collapse1");
const collapse = new Collapse(collapseEl);

collapseEl.addEventListener("expanded", (event) => {
  console.log("Collapse is open", event.detail);
});

collapseEl.addEventListener("collapsed", (event) => {
  console.log("Collapse is closed", event.detail);
});
```
