...
Update the dropdown placement and offset without immediately showing it.

```js
myDropdown.setOptions({ placement: Placement, offsetDistance: number });
```

### `cleanup()`

Call `cleanup()` when the dropdown is removed from the page.
...
The dropdown dispatches events on the dropdown content element:

- `dropdown-show`
- `dropdown-hide`

```js
const dropdownEl = document.querySelector("#myDropdown");

new Dropdown(dropdownEl);

dropdownEl.addEventListener("dropdown-show", (event) => {
  console.log(event.detail.isHidden); // false
});

dropdownEl.addEventListener("dropdown-hide", (event) => {
