...
<Tabs values={["Preview", "Code"]} autoHeight>
  ...
  <TabPanel value="Code">
    <CodeLoader source="TooltipDefaults"/>
  </TabPanel>
</Tabs>
...
Call `cleanup()` when the tooltip is removed from the page.
...
The tooltip dispatches events on the tooltip element:
...
tooltipEl.addEventListener("tooltip-show", (event) => {
  console.log(event.detail.isHidden); // false
});

tooltipEl.addEventListener("tooltip-hide", (event) => {
