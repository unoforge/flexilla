...
<Tabs values={["Preview", "Code"]} autoHeight>
  ...
  <TabPanel value="Code">
    <CodeLoader source="AccordionExample1"/>
  </TabPanel>
</Tabs>
...
const options = {
  ...
  allowCloseFromContent: false, // default is false
...
<BoxTable>
  ...
  <BoxTableRow title="allowCloseFromContent" attribute="data-allow-close-from-content" type="boolean | undefined">
    Allows the content element to close the current item when clicked.
  </BoxTableRow>
...
To show a specific item in the accordion, use `show(id)`.
...
myAccordion.show("value-of-the-concerned-item");
```

### `hide(id)`

To hide a specific item, use `hide(id)`.

```js
myAccordion.hide("value-of-the-concerned-item");
```

### `cleanup()`

Call `cleanup()` when the accordion is removed from the page.
...
The accordion dispatches `change-item` on the accordion root element whenever the active item changes.

```js
const faqEl = document.querySelector("#accordion-example");
